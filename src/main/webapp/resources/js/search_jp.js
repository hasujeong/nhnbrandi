...
function fn_search() {
	...
	if (today_yn == "Y") {
		option_str = option_str + "&filter.is_today_delivery=1"
	}
	
	if (sale_yn == "Y") {
		option_str = option_str + "&&trigger._sale_percent=value,sale_percent&filter._sale_percent=[" + $('#range-slider-input').val() + ",]"
		
		//filter.is_sell=1&filter.is_exhibition=1&trigger._sale_percent=value,sale_percent&filter._sale_percent=[$PERCENT,]
	}
	...
	param = param + "&q_option=and,shopping_idx&search_tp=sb&pr=pc&format=v2&filter.is_sell=1&filter.is_exhibition=1";
	...
	if (depth == "M") {
		...
		$.each(category_123, function(index, cate) {
			if (cate.id != '') {
				...
				if (((cate_name[0]).split('_')[1] + '\u003e' + (cate_name[1]).split('_')[1]) == key2) {
...
function fn_reset() {
	...
	$('.range-slider').css('--value', '0');
	$('.range-slider').css('--text-value', '0');
	$('#range-slider-input').val('0');
	$('#sale_text').html('0%');
